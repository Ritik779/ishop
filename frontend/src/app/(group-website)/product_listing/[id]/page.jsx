...
import { getProductByID } from "@/library/api-calls";
...
export default function ProductListing() {
  ...
  const [mainImage, setMainImage] = useState(product?.main_image); // State for Main Image
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div>
          ...
          <div className="flex gap-3 mt-4 p-2">
            {product?.other_images &&
              [product.main_image, ...product.other_images].map((img, index) => (
                <img
                  key={index}
                  src={`${process.env.NEXT_PUBLIC_BASE_URL_IMG}/product/${img}`}
                  alt={`Thumbnail ${index + 1}`}
                  className={`h-20 rounded-lg border p-1 cursor-pointer object-cover ${mainImage === img ? "border-blue-500" : "border-gray-300"
                    }`}
        ...
        <div>
          ...
          <div className="flex items-center gap-3 my-3">
            ...
            <span className="text-gray-400 line-through">${product?.orignal_price}</span>
          ...
          <div className="max-w-[400px]">
            ...
            <CartBtn colors={product?.color} product_id={product?._id} prices={{ original_price: product?.original_price, discounted_price: product?.discounted_price }} />
          </div>
        </div>
      </div>
    </div>
